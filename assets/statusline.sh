...
if [ -z "$input" ]; then
    printf "\033[38;2;255;50;50m\033[0m Daemon"
    exit 0
fi

# ── Colors ── Hellfire palette ──────────────────────────
blood='\033[38;2;200;0;30m'
hellfire='\033[38;2;180;30;20m'
ember='\033[38;2;150;20;15m'
sulfur='\033[38;2;120;15;10m'
ash='\033[38;2;70;70;70m'
bone='\033[38;2;100;100;100m'
ghost='\033[38;2;140;140;140m'
smoke='\033[38;2;85;85;85m'
dim='\033[2m'
bold='\033[1m'
reset='\033[0m'
...
color_for_pct() {
    local pct=$1
    if [ "$pct" -ge 90 ]; then printf "$blood"
    elif [ "$pct" -ge 70 ]; then printf "$hellfire"
    elif [ "$pct" -ge 50 ]; then printf "$ember"
    elif [ "$pct" -ge 30 ]; then printf "$bone"
    else printf "$ash"
    fi
}
...
build_bar() {
    ...
    printf "${bar_color}${filled_str}${ash}${empty_str}${reset}"
}
...
if [[ "$parent_cmd" == *"--dangerously-skip-permissions"* ]]; then
    skip_perms="${blood} ${reset} "
fi

ctx_icon=$(threat_icon_for_pct "$pct_used")

line1="${smoke}${model_name}${reset}"
line1+="${sep}"
line1+="${pct_color}${pct_used}%${reset}"
line1+="${sep}"
line1+="${skip_perms}${bone}${dirname}${reset}"
if [ -n "$git_branch" ]; then
    line1+=" ${ash}${git_branch}${reset}"
    [ -n "$git_dirty" ] && line1+="${blood}*${reset}"
fi
if [ -n "$session_duration" ]; then
    line1+="${sep}"
    line1+="${ash}${session_duration}${reset}"
fi
line1+="${sep}"
case "$effort" in
    high)   line1+="${hellfire}${effort}${reset}" ;;
    medium) line1+="${ash}${effort}${reset}" ;;
    low)    line1+="${ash}${effort}${reset}" ;;
    *)      line1+="${ash}${effort}${reset}" ;;
esac
...
if [ -n "$five_hour_pct" ]; then
    ...
    five_icon=$(threat_icon_for_pct "$five_hour_pct")

    rate_lines+="${ash}5h${reset} ${five_hour_bar} ${five_hour_pct_color}${five_hour_pct_fmt}%${reset}"
    [ -n "$five_hour_reset" ] && rate_lines+=" ${ash}${five_hour_reset}${reset}"
fi
...
if [ -n "$seven_day_pct" ]; then
    ...
    rate_lines+="${ash}7d${reset} ${seven_day_bar} ${seven_day_pct_color}${seven_day_pct_fmt}%${reset}"
    [ -n "$seven_day_reset" ] && rate_lines+=" ${ash}${seven_day_reset}${reset}"
fi
...
if [ "$extra_enabled" = "true" ] && [ -n "$usage_data" ]; then
    ...
    [ -n "$rate_lines" ] && rate_lines+="\n"
    rate_lines+="${ash}$$${reset} ${extra_bar} ${extra_pct_color}\$${extra_used}${ash}/${reset}${ghost}\$${extra_limit}${reset} ${ash}${extra_reset}${reset}"
fi
